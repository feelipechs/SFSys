...
const PASSWORD_LOWERCASE_REGEX = /(?=.*[a-z])/; // pelo menos uma letra minúscula
const PASSWORD_UPPERCASE_REGEX = /(?=.*[A-Z])/; // pelo menos uma letra maiúscula
const PASSWORD_NUMBER_REGEX = /(?=.*\d)/; // pelo menos um dígito
const PASSWORD_SYMBOL_REGEX = /(?=.*[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?~`])/; // pelo menos um símbolo

// funções de validação (adaptadas para o RHF)
export const FormValidators = {
   ...
   */
  email: (value) => {
    // o 'required' do RHF deve lidar com o campo vazio, mas é bom veirifcar
   ...
   */
  password: (value) => {
    // permite que o RHF lide com 'required: true'
    if (!value) return true;
    ...
    // símbolo (opcional, mas recomendado)
